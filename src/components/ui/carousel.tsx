...
export const SliderProgress = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      <div
        className="dark:bg-white bg-slate-800 absolute w-full top-0 -left-[100%] bottom-0"
...
export const SliderSnapDisplay = ({ className }: { className?: string }) => {
  const { selectedSnap, snapCount, direction }: any = useCarouselContext();
            ...
            {index === selectedIndex && (
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="absolute z-[3] w-full h-full left-0 top-0 dark:bg-white bg-slate-800 rounded-full"
                />
              </AnimatePresence>
            )}
          </button>
